...
const Sidebar = () => {
  ...
  const fetchSidebarData = async () => {
    try {
      ...
      // Fetch categories
      const categoriesResponse = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.cleanairindia.com/api'}/categories`, {
        headers: {
      ...
      // Fetch recent posts
      const postsResponse = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.cleanairindia.com/api'}/blogs?page=1&limit=3&status=published`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      // Fetch tags
      const tagsResponse = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.cleanairindia.com/api'}/tags`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      <div className="letest-blog-card">
          ...
          {/* <div className="letest-blog-search-section">
            <form onSubmit={handleSearch} method="post">
              ...
              />
              <button type="submit">
                <img height="16" width="16" src={SerachIcon} alt="Search" loading="lazy"/>
              </button>
            </form>
          </div> */}
            ...
            <ul>
              {recentPosts.map((post) => (
                <li key={post._id}>
                  ...
                  >
                    <img src={getBlogImageUrl(post.featuredImage)} alt="post thumbnail" loading="lazy"/>
                  </NavLink>
