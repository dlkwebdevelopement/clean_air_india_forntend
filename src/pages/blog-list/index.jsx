...
const BlogList = () => {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.cleanairindia.com/api'}/blogs?${params}`, {
  ...
  const fetchAllBlogsForStats = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.cleanairindia.com/api'}/blogs?limit=1000`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleBulkDelete = async (blogIds) => {
    try {
      ...
      const deletePromises = blogIds.map(id => 
        fetch(`${import.meta.env.VITE_API_URL || 'https://api.cleanairindia.com/api'}/blogs/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
